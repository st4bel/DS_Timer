    ...
    for file in os.listdir(schedule_path):
        if os.path.isfile(os.path.join(schedule_path, file)):
            with open(os.path.join(schedule_path, file)) as fd:
                ...
                action["source_village_name"] = world_data.get_village_name_from_id(
                    action["domain"], action["source_id"])
                action["target_village_name"] = world_data.get_village_name_from_id(
                    action["domain"], action["target_id"])
...
def new_atts_get():
    ...
    return render_template("new_attack.html", templates=get_templates(), unitnames=get_unitnames(),
                           players=players, N_O_P=len(players))
